...
func (l *Loki) Process(query string, start, end time.Time, limit int64) {
	// split into smaller time windows
	windows := calcWindows(start.UTC().UnixNano(), end.UTC().UnixNano(), time.Hour.Nanoseconds())
		...
		for {
			...
			for _, s := range ir {
				...
				for k, v := range s.Labels {
					if k != "host" && k != "job" && k != "type" {
						continue
					}
					ls[model.LabelName(k)] = model.LabelValue(v)
				}
				for _, e := range s.Entries {
					l.lokiClient.Handle(ls, e.Timestamp, e.Line)
